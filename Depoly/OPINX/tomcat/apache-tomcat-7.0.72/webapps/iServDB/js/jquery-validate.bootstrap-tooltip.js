        ...
        $.each(this.errorList, function(index, value) {
          ...
          if (bsMajorVer === 3 && bsMinorVer >= 3) {
            ...
            if (currentElement.data('bs.tooltip') !== undefined) {
              currentElement.data('bs.tooltip').options.title = i18n._(value.message);
            } else {
              currentElement.tooltip(_this.applyTooltipOptions(value.element, i18n._(value.message)));
            }
            $(value.element).removeClass(_this.settings.validClass).addClass(_this.settings.errorClass).tooltip('show');
          } else {
            $(value.element).removeClass(_this.settings.validClass).addClass(_this.settings.errorClass).tooltip('destroy').tooltip(_this.applyTooltipOptions(value.element, i18n._(value.message))).tooltip('show');
          }
