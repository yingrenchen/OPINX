...
def insertDataToPG(d,RC_price,Current_transaction,AC_transaction,code):
	...
	# print (type(change))
	# print (type(code))
	cur.execute('CREATE TABLE IF NOT EXISTS tw_all (d timestamp,RC_price double precision NOT NULL,Current_transaction double precision NOT NULL,AC_transaction double precision NOT NULL,code TEXT NOT NULL);')
	#cur.execute('CREATE INDEX index_d on tw_'+code+' (d);')
	cur.execute("INSERT INTO tw_all (d,rc_price,current_transaction,ac_transaction,code)VALUES(%s,%s,%s,%s,%s)",(d,RC_price,Current_transaction,AC_transaction,code))
	conn.commit()
